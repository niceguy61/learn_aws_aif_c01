...
- 선수 지식: **사전 훈련된 서비스**는 일반적인 작업을 위해 모델 훈련을 직접 하지 않고 API로 기능을 호출하는 방식입니다.
- 한 줄 요약: **이미지·문서·텍스트·음성의 입력과 원하는 출력을 먼저 말하면 알맞은 관리형 AI 서비스를 고를 수 있습니다.**

### 자주 하는 오해

- **오해:** 모든 AI 요구사항에 사용자 지정 모델 훈련이 필요하다.
- **바로잡기:** 일반적인 비전·문서 추출·감정 분석·음성 인식은 관리형 서비스부터 검토하는 것이 합리적입니다.

### 스스로 답하는 확인 질문

1. 스캔 문서의 표와 양식을 추출하려면 어떤 서비스가 알맞나요?
2. 회의 녹음을 텍스트와 자막으로 바꾸는 요구사항의 입력과 출력은 무엇인가요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 1 Task 1.2의 실제 AI 애플리케이션과 AWS 관리형 AI/ML 서비스 기능에 연결됩니다.
