...
- 선수 지식: **결정론적** 시스템은 같은 입력에 같은 결과를 내고, ML은 보통 확률·점수 형태의 예측을 냅니다.
- 한 줄 요약: **AI는 대규모 패턴·예측 문제에 유용하지만, 비용보다 이점이 작거나 규칙의 일관성이 필수라면 먼저 규칙 기반 대안을 검토합니다.**

### 자주 하는 오해

- **오해:** AI를 쓰면 모든 업무의 품질과 비용이 자동으로 좋아진다.
- **바로잡기:** 데이터 품질, 구축·운영 비용, 설명 가능성, 오류의 영향까지 비교해야 합니다.

### 스스로 답하는 확인 질문

1. 동일한 세금 계산 결과를 항상 보장해야 한다면 AI와 규칙 기반 중 무엇을 먼저 선택하나요?
2. 수백만 건 센서 기록에서 드문 고장 패턴을 찾는 문제는 왜 ML 후보인가요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 1 Task 1.2의 AI/ML 가치 판단과 부적합 사례 판단에 연결됩니다.
