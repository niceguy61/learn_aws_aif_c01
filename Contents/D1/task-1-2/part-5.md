...
- 선수 지식: 사례의 숫자는 특정 시점·조건에서 나온 결과일 수 있으므로, 서비스 선택의 일반 규칙과 분리해 읽어야 합니다.
- 한 줄 요약: **고객 사례에서는 회사 이름보다 문제 유형, 입력 데이터, 필요한 결과, 연결된 AWS 서비스를 순서대로 읽습니다.**

### 자주 하는 오해

- **오해:** 고객 사례에 나온 성과 수치가 모든 조직에서 재현된다.
- **바로잡기:** 성과는 데이터·운영 환경·평가 기준에 따라 달라집니다. 여기서는 문제와 서비스의 연결을 학습합니다.

### 스스로 답하는 확인 질문

1. 콜센터 통화량처럼 시간에 따라 변하는 수요를 예측하는 사례의 핵심 데이터 유형은 무엇인가요?
2. 사진으로 비슷한 상품을 찾는 서비스는 어떤 AI 애플리케이션 범주에 속하나요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 1 Task 1.2의 실제 AI 애플리케이션 예시 식별에 연결됩니다.
- **AWS 실무 확장:** 고객 성과 수치와 구현 세부 사항은 사례 발표 시점에 의존하므로 공식 사례 출처로 별도 검증이 필요합니다.
