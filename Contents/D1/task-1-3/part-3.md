...
- 선수 지식: 파라미터는 훈련 중 모델이 학습하는 값이고, 하이퍼파라미터는 훈련 전에 사람이 정하는 설정입니다.
- 한 줄 요약: **여러 설정을 공정하게 비교하고 목표 지표를 기준으로 선택하는 과정이 모델 훈련·평가입니다.**

### 자주 하는 오해

- **오해:** AUC나 정확도 하나만 높으면 즉시 배포해도 된다.
- **바로잡기:** 지표는 문제와 비용을 함께 반영해야 하며, 새 데이터와 운영 조건에서도 평가해야 합니다.

### 스스로 답하는 확인 질문

1. 학습률과 에포크 수는 파라미터와 하이퍼파라미터 중 무엇인가요?
2. 여러 훈련 실행의 결과를 비교해 최적 설정을 찾는 작업은 왜 필요한가요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 1 Task 1.3의 MLOps 기본 개념과 모델·비즈니스 지표 설명에 연결됩니다.
