...
- 선수 지식: ML 수명 주기는 모델을 배포한 뒤에도 데이터와 성능을 계속 확인하는 반복 과정입니다.
- 한 줄 요약: **ML 프로젝트는 모델부터 만들지 않고, 측정 가능한 비즈니스 목표와 가장 단순한 해결책부터 검토합니다.**

### 자주 하는 오해

- **오해:** 모델을 배포하면 ML 프로젝트가 끝난다.
- **바로잡기:** 배포 후 성능·데이터 변화·비즈니스 결과를 모니터링하고 필요하면 재훈련해야 합니다.

### 스스로 답하는 확인 질문

1. ML 프로젝트에서 모델 훈련보다 먼저 정의해야 하는 것은 무엇인가요?
2. 이미 사용할 수 있는 관리형 AI 서비스가 있다면 처음부터 모델을 훈련하기 전에 무엇을 검토하나요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 1 Task 1.3의 파이프라인 구성 요소와 모델 소스 설명에 연결됩니다.
