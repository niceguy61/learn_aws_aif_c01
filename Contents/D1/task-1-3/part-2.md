...
- 선수 지식: ETL은 여러 곳의 데이터를 꺼내고, 필요한 형태로 바꾸고, 사용할 위치에 저장하는 과정입니다.
- 한 줄 요약: **좋은 모델 이전에 데이터 품질·레이블·분할·특성 선택을 반복 가능하게 준비해야 합니다.**

### 자주 하는 오해

- **오해:** 80/10/10은 모든 데이터셋에 반드시 적용해야 하는 고정 규칙이다.
- **바로잡기:** 이는 학습용 예시 비율입니다. 데이터 양·시간 순서·업무 위험에 맞는 분할 전략이 필요합니다.

### 스스로 답하는 확인 질문

1. 원본 데이터가 아닌 위치와 스키마 같은 메타데이터를 관리하는 구성 요소는 무엇인가요?
2. 지도 학습용 대규모 레이블 데이터를 준비할 때 어떤 서비스 유형을 고려하나요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 1 Task 1.3의 AI/ML 파이프라인 단계별 AWS 서비스 식별에 연결됩니다.
- **AWS 실무 확장:** 변환 수와 인력 규모 같은 변동 수치는 개별 서비스 공식 문서로 추가 검증이 필요합니다.
