...
- 선수 지식: 전이 학습은 사전 훈련된 모델을 출발점으로 삼아 새 작업에 맞추는 방법입니다.
- 한 줄 요약: **AWS GenAI 서비스의 가치는 모델·인프라를 직접 처음부터 운영하지 않고도, 보안과 운영 요구를 고려해 애플리케이션을 빠르게 만들 수 있다는 점입니다.**

### 자주 하는 오해

- **오해:** 관리형 AI 서비스를 쓰면 보안과 책임을 전부 AWS가 맡는다.
- **바로잡기:** AWS와 고객의 책임은 나뉘며, 고객은 데이터·접근 권한·애플리케이션 사용 방식을 계속 관리해야 합니다.

### 스스로 답하는 확인 질문

1. 사전 훈련된 모델을 새 조직의 작업에 맞추는 방법은 무엇인가요?
2. 프롬프트 인젝션은 입력·모델·출력 중 어느 경로를 노릴 수 있는 위험인가요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 2 Task 2.3의 AWS GenAI 서비스·인프라 이점과 보안·책임 이해에 연결됩니다.
