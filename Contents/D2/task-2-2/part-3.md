...
![파운데이션 모델 커스터마이징 스펙트럼](../../../assets/images/d2-model-customization-spectrum.svg)

```mermaid
flowchart LR
  subgraph Spectrum["FM 커스터마이징 4단계 스펙트럼"]
    direction TB
    P["1. 프롬프트 엔지니어링<br/>비용: $<br/>가중치 불변 / 즉시 도입"]
    R["2. RAG (검색 증강 생성)<br/>비용: $$<br/>사내 지식 연결 / 최신 정보"]
    F["3. 미세 조정 (Fine-Tuning)<br/>비용: $$$<br/>특화 문체·포맷 / 라벨 데이터"]
    C["4. 사전 훈련 (Pre-training)<br/>비용: $$$$$<br/>초거대 연산 / 독자 파운데이션"]
    P --> R --> F --> C
  end
```
...
- 선수 지식: KPI는 기술 성능뿐 아니라 고객·비용·운영 목표의 달성 정도를 보는 측정값입니다.
- 한 줄 요약: **모델 선택은 품질만 비교하지 않고 기능, 제약, 규정 준수, 운영 비용과 비즈니스 KPI를 함께 평가하는 결정입니다.**

### 자주 하는 오해

- **오해:** 정확도가 가장 높은 모델이 항상 최선이다.
- **바로잡기:** 응답 시간, 비용, 일관성, 안전성, 전환율 같은 사용 사례의 성공 기준도 함께 비교해야 합니다.

### 스스로 답하는 확인 질문

1. 고객 지원 챗봇에서 정확도 외에 추적할 수 있는 비즈니스 지표는 무엇인가요?
2. 최신 사내 지식이 필요한 경우 프롬프트, RAG, 미세 조정 중 무엇을 먼저 검토할 수 있나요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 2 Task 2.2의 모델 선택 요소와 비즈니스 가치·지표 결정에 연결됩니다.
