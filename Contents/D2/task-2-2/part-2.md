...
- 선수 지식: 환각은 모델이 그럴듯하지만 근거 없는 내용을 생성하는 위험입니다.
- 한 줄 요약: **미세 조정과 인간 피드백은 응답 품질을 개선할 수 있지만, 사실성·안전성 검증을 대신하지는 않습니다.**

### 자주 하는 오해

- **오해:** RLHF를 적용하면 모델은 더 이상 환각하지 않는다.
- **바로잡기:** 정렬 기법은 선호도와 안전성 개선에 도움을 주지만, 결과 검증과 보호 통제가 계속 필요합니다.

### 스스로 답하는 확인 질문

1. 요약 품질과 번역 품질을 비교할 때 각각 어떤 지표를 고려할 수 있나요?
2. 중요한 건강 안내를 LLM이 만들었다면 왜 권위 있는 출처로 다시 확인해야 하나요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 2 Task 2.2의 GenAI 한계와 성능·비즈니스 지표 이해에 연결됩니다.
