...
- 선수 지식: RAG는 검색한 외부 정보를 모델의 컨텍스트로 제공하는 접근입니다.
- 한 줄 요약: **GenAI는 적응성·대화·콘텐츠 생성에 강점이 있지만, 최신·사내 정보와 고위험 판단에는 컨텍스트와 사람 검토가 필요합니다.**

### 자주 하는 오해

- **오해:** 대화형 모델은 이전 대화를 영구적으로 기억한다.
- **바로잡기:** 애플리케이션이 필요한 정보를 컨텍스트나 메모리로 제공하지 않으면 모델이 지속 기억을 보장하지 않습니다.

### 스스로 답하는 확인 질문

1. 최신 사내 정책에 답해야 하는 챗봇에 외부 문서를 제공하는 이유는 무엇인가요?
2. 일반적인 이메일 요약과 법률 판단 중 사람 검토가 더 중요한 경우는 무엇인가요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 2 Task 2.2의 GenAI 장점·한계와 적합성 판단에 연결됩니다.
