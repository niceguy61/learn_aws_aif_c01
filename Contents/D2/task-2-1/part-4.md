...
- 선수 지식: 사용 사례는 입력, 원하는 출력, 품질·안전 제약으로 설명할 수 있습니다.
- 한 줄 요약: **생성형 AI는 생성·요약·번역·코드 같은 다양한 작업에 쓰이지만, 목표와 데이터 형식에 맞는 모델·도구를 선택해야 합니다.**

### 자주 하는 오해

- **오해:** 한 생성 모델이 모든 콘텐츠 유형에 같은 품질로 적합하다.
- **바로잡기:** 텍스트·이미지·코드 등 목표와 입력·출력 형식에 따라 적합한 모델과 평가 방법이 다릅니다.

### 스스로 답하는 확인 질문

1. 긴 정책 문서의 핵심만 짧게 제공하려면 어떤 생성형 태스크를 선택하나요?
2. 자연어 설명에서 함수 초안을 만드는 것은 어떤 사용 사례인가요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 2 Task 2.1의 GenAI 사용 사례 식별에 연결됩니다.
