...
- 선수 지식: 토큰은 모델이 텍스트를 처리하는 단위이고, 임베딩은 의미를 숫자 벡터로 나타낸 표현입니다.
- 한 줄 요약: **토큰화와 임베딩은 입력을 모델이 계산할 수 있는 숫자 표현으로 바꾸고, 트랜스포머는 문맥 관계를 활용해 다음 출력을 만듭니다.**

### 자주 하는 오해

- **오해:** 임베딩은 단순히 단어를 번호로 바꾸는 작업이다.
- **바로잡기:** 임베딩은 의미적 관계를 담는 수치 표현이며, 유사한 의미의 항목은 벡터 공간에서도 가까워질 수 있습니다.

### 스스로 답하는 확인 질문

1. 토큰과 임베딩은 생성형 AI 입력 처리에서 각각 어떤 역할을 하나요?
2. 문서 검색에서 의미가 비슷한 문장을 찾고 싶다면 왜 임베딩을 사용할 수 있나요?

### 공식 범위와 출처

- **시험 핵심:** AIF-C01 Domain 2 Task 2.1의 토큰·임베딩·벡터·트랜스포머 기반 LLM 기본 개념에 연결됩니다.
