...
![정책, 통제, 증적, 검토와 개선이 순환하는 거버넌스 증적 주기](../../../assets/images/d5-governance-evidence-cycle.svg)

이 그림은 규정 준수와 거버넌스를 문서 하나가 아니라 정책·통제·증적·검토의 반복 과정으로 설명합니다. 특정 인증의 충족 여부는 이 그림만으로 판단할 수 없으며, 조직의 요구 사항과 공식 감사 기준을 따라야 합니다.
...
- **한 줄 요약:** 규정 준수는 AWS의 인증 보고서를 활용하되, 고객이 자신의 AI 워크로드에 필요한 통제와 증적을 구성·운영하는 활동입니다.
- **선수 지식:** 감사 증적은 정책이나 통제가 실제로 작동했음을 보여 주는 기록·보고서·로그입니다.

### 자주 하는 오해

- **오해:** AWS Artifact에서 SOC 또는 ISO 보고서를 받으면 고객 시스템도 자동으로 규정을 준수한다.
  **바로잡기:** Artifact는 AWS의 통제에 관한 보고서를 제공합니다. 고객은 데이터·권한·애플리케이션 구성 등 자신의 책임 범위를 별도로 입증해야 합니다.

### 스스로 답하는 확인 질문

1. 공동 책임 모델이 규정 준수에도 적용되는 이유는 무엇인가요?
2. AWS Artifact와 고객의 자체 감사 증적은 어떤 차이가 있나요?

### 공식 범위와 출처

- **시험 핵심:** 공식 Task 5.2의 거버넌스·규정 준수 지원 서비스와 공동 책임 범위를 다룹니다.
