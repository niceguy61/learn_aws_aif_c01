...
- **한 줄 요약:** 데이터 품질·접근 제어·보존 기간·저장 비용을 함께 관리해야 AI 학습 데이터가 신뢰 가능하고 규정에 맞게 유지됩니다.
- **선수 지식:** 데이터 수명 주기는 데이터를 생성·사용·보관·아카이브·삭제하는 전체 기간입니다.

### 자주 하는 오해

- **오해:** 비용을 줄이려면 오래된 훈련 데이터를 바로 삭제하면 된다.
  **바로잡기:** 보존 의무와 재현성 요구가 있을 수 있습니다. 접근 패턴·보존 정책·감사 요구에 맞춰 S3 등급과 수명 주기 규칙을 설계합니다.

### 스스로 답하는 확인 질문

1. Lake Formation의 세분화된 권한은 데이터 레이크에서 어떤 문제를 줄이나요?
2. S3 Intelligent-Tiering과 Glacier는 각각 어떤 접근 패턴에 적합한가요?

### 공식 범위와 출처

- **시험 핵심:** 공식 Task 5.2의 데이터 수명 주기, 보존, 데이터 액세스 제어와 데이터 거버넌스 전략을 다룹니다.
