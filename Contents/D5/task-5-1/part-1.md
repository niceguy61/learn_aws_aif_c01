...
- **한 줄 요약:** 공동 책임 모델에서 AWS는 클라우드 자체를 보호하고, 고객은 데이터·권한·서비스 구성을 보호합니다.
- **선수 지식:** IAM은 AWS 리소스에 누가 어떤 작업을 할 수 있는지 관리하는 서비스입니다.

### 자주 하는 오해

- **오해:** 루트 사용자는 모든 권한이 있으므로 일상 작업에 가장 편리하다.
  **바로잡기:** 루트 사용자는 권한을 제한할 수 있어 위험이 큽니다. MFA를 적용하고 일상 작업은 필요한 권한만 가진 역할 또는 사용자로 수행합니다.

### 스스로 답하는 확인 질문

1. EC2에서 직접 모델을 운영할 때와 완전관리형 추론 서비스를 사용할 때 고객 책임은 어떻게 달라지나요?
2. MFA가 암호만 사용하는 인증보다 강한 이유는 무엇인가요?

### 공식 범위와 출처

- **시험 핵심:** 공식 Task 5.1의 IAM 역할·정책·권한과 AWS 공동 책임 모델 범위를 다룹니다.
