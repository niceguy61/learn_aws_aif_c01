...
- **한 줄 요약:** 암호화는 저장·전송 중 데이터를 보호하고, KMS는 키 사용을 통제하며, VPC와 PrivateLink는 네트워크 경계를 좁힙니다.
- **선수 지식:** 저장 시 암호화는 저장된 데이터를, 전송 중 암호화는 네트워크를 오가는 데이터를 보호합니다.

### 자주 하는 오해

- **오해:** 데이터를 암호화하면 누구나 읽기 권한을 받아도 안전하다.
  **바로잡기:** 암호화는 접근 제어를 대체하지 않습니다. 특히 KMS 키 사용 권한, IAM 권한, 네트워크 경계를 함께 관리해야 합니다.

### 스스로 답하는 확인 질문

1. 클라이언트 측과 서버 측 암호화는 데이터를 암호화하는 시점이 어떻게 다른가요?
2. PrivateLink를 쓰는 VPC 전용 구성은 어떤 네트워크 노출을 줄이나요?

### 공식 범위와 출처

- **시험 핵심:** 공식 Task 5.1의 암호화, Amazon Macie, AWS PrivateLink, 보안 데이터 엔지니어링 범위를 다룹니다.
