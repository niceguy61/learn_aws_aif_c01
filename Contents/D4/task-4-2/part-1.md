...
- **한 줄 요약:** 해석 가능성은 모델 내부 작동을 이해하는 정도이고, 설명 가능성은 복잡한 모델의 특정 결과를 사람이 이해할 말로 설명하는 접근입니다.
- **선수 지식:** 블랙박스 모델은 내부 계산 과정을 직접 이해하기 어려운 복잡한 모델을 가리키는 표현입니다.

### 자주 하는 오해

- **오해:** 설명 가능성은 모델의 소스 코드와 훈련 데이터를 전부 공개하는 일이다.
  **바로잡기:** 설명은 특정 결정의 이유와 한계를 전달하는 것이며, 공개 범위는 개인정보·보안·독점 정보 보호와 균형을 맞춰 정합니다.

### 스스로 답하는 확인 질문

1. 규제상 결정 근거를 명확히 제시해야 하는 업무에서 단순 모델을 선택할 수 있는 이유는 무엇인가요?
2. 투명성을 높일 때 안전성과 개인정보 보호 측면에서 어떤 위험이 생길 수 있나요?

### 공식 범위와 출처

- **시험 핵심:** 공식 Task 4.2의 투명·설명 가능 모델 구분과 모델 안전성·투명성의 절충점을 다룹니다.
