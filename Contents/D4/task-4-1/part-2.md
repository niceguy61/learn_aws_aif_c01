...
- **한 줄 요약:** SageMaker Clarify는 데이터와 모델 결과를 집단별로 비교해 편향 신호와 특성 기여도를 찾아보는 데 도움을 주는 도구입니다.
- **선수 지식:** 재현율은 실제 긍정 사례를 얼마나 놓치지 않고 찾았는지, 특이도는 실제 부정 사례를 얼마나 정확히 부정으로 판단했는지 보는 지표입니다.

### 자주 하는 오해

- **오해:** Clarify가 편향 점수를 보여 주면 편향이 자동으로 제거된다.
  **바로잡기:** Clarify는 탐지와 분석을 돕습니다. 데이터 보완, 레이블 검토, 기준 재설계, 인간 검토 등 완화 조치는 별도로 결정하고 검증해야 합니다.

### 스스로 답하는 확인 질문

1. 두 집단의 전체 정확도가 같아도 처리 평등 문제가 남을 수 있는 이유는 무엇인가요?
2. 데이터 준비 단계와 배포 후 단계 모두에서 편향을 점검해야 하는 이유는 무엇인가요?

### 공식 범위와 출처

- **시험 핵심:** 공식 Task 4.1의 편향·신뢰성·진실성 탐지와 모니터링 도구(SageMaker Clarify 포함)를 다룹니다.
