...
- **한 줄 요약:** 책임 있는 AI는 공정성·견고성·개인정보 보호·거버넌스·투명성을 대표성 있는 데이터와 지속적 검토로 함께 관리합니다.
- **선수 지식:** PII(개인 식별 정보)는 개인을 직접 또는 간접으로 식별할 수 있는 정보이며, 클래스 불균형은 특정 집단의 데이터가 다른 집단보다 적은 상태입니다.

### 자주 하는 오해

- **오해:** 데이터에 민감 속성을 넣지 않으면 편향은 사라진다.
  **바로잡기:** 다른 특성이 민감 속성을 대신할 수 있고, 집단별 데이터 수·레이블·결과 차이도 편향을 만들 수 있으므로 결과를 함께 점검해야 합니다.

### 스스로 답하는 확인 질문

1. 클래스 불균형이 의료 또는 대출 모델의 특정 집단에 어떤 영향을 줄 수 있나요?
2. 모델의 탄소 발자국과 재사용 가능한 사전 훈련 모델은 왜 책임 있는 선택과 연결되나요?

### 공식 범위와 출처

- **시험 핵심:** 공식 Task 4.1의 책임 있는 AI 특성, 모델 선택 관행, 데이터세트 특성, 편향과 변동 영향을 다룹니다.
