...
- **한 줄 요약:** 생성형 AI의 할루시네이션·유해 콘텐츠·편향·저작권·민감 정보 위험은 Guardrails, 평가, 인간 검토를 함께 사용해 줄여야 합니다.
- **선수 지식:** 할루시네이션은 모델이 그럴듯하지만 근거 없거나 틀린 내용을 생성하는 현상입니다.

### 자주 하는 오해

- **오해:** Amazon Bedrock Guardrails를 적용하면 사실성·저작권·개인정보 위험이 모두 자동으로 해결된다.
  **바로잡기:** Guardrails는 구성한 정책에 따라 입력과 출력을 필터링하는 방어층입니다. 근거 검증, 데이터 최소화, 권한 통제, 평가·모니터링이 함께 필요합니다.

### 스스로 답하는 확인 질문

1. 사용자 입력이 금지 주제를 요청할 때와 모델 출력이 유해할 때 Guardrails는 각각 어디에서 작동하나요?
2. 사실적 지식과 시맨틱 견고성 평가는 서로 어떤 다른 위험을 찾나요?

### 공식 범위와 출처

- **시험 핵심:** 공식 Task 4.1의 생성형 AI 법적 위험, Amazon Bedrock Guardrails, 편향·신뢰성·진실성 평가 범위를 다룹니다.
