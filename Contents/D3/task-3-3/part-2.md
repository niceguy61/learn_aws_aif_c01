...
- **한 줄 요약:** 좋은 미세 조정은 대표성 있는 데이터를 준비해 훈련·검증·테스트로 나누고, 최종 테스트는 마지막에 한 번 사용해 일반화 성능을 확인합니다.
- **선수 지식:** 훈련 데이터는 가중치 학습에 쓰고, 검증 데이터는 설정 비교에 쓰며, 테스트 데이터는 최종 확인용으로 분리합니다.

### 자주 하는 오해

- **오해:** 데이터가 많기만 하면 미세 조정 품질이 보장된다.
  **바로잡기:** 데이터의 대표성, 레이블 품질, 편향, 권한·거버넌스, 훈련·검증·테스트 분리가 모두 결과에 영향을 줍니다.

### 스스로 답하는 확인 질문

1. 테스트 데이터를 모델 설정을 고를 때 반복 사용하면 왜 문제가 될까요?
2. 데이터 편향 탐지와 레이블 지정 업무에 각각 어떤 AWS 도구를 검토할 수 있나요?

### 공식 범위와 출처

- 공식 Task 3.3의 미세 조정 데이터 준비(큐레이션, 거버넌스, 크기, 레이블, 대표성, 인간 피드백) 범위를 다룹니다.
