...
- **한 줄 요약:** 미세 조정은 준비된 FM의 행동을 특정 업무 예시로 바꾸는 방법이며, 비용과 데이터 품질을 고려해 프롬프트·RAG보다 필요한 경우에 선택합니다.
- **선수 지식:** 가중치는 모델이 학습한 조절값이고, 레이블은 훈련 예시에서 기대하는 정답 또는 원하는 출력입니다.

### 자주 하는 오해

- **오해:** 최신 사내 문서를 답하게 하려면 항상 미세 조정해야 한다.
  **바로잡기:** 자주 바뀌는 사실 지식은 RAG가 더 적합할 수 있습니다. 미세 조정은 일관된 형식·업무 행동·전문 표현을 학습시켜야 할 때 검토합니다.

### 스스로 답하는 확인 질문

1. 전체 미세 조정 대신 PEFT나 LoRA를 검토하는 비용·운영상 이유는 무엇인가요?
2. 단일 업무만 학습한 모델에서 치명적 망각이 어떤 문제를 만들 수 있나요?

### 공식 범위와 출처

- 공식 Task 3.3의 사전 훈련, 미세 조정, 지속적 사전 훈련, 지침 튜닝과 도메인 적응 범위를 다룹니다.
