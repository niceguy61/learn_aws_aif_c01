...
- **한 줄 요약:** FM 평가는 자동 점수 하나가 아니라 과제 품질·안전성·인간 선호·지연 시간·비용을 비즈니스 목표와 함께 확인하는 과정입니다.
- **선수 지식:** 벤치마크는 여러 모델을 같은 기준으로 비교하는 평가 세트이고, 비결정적 출력은 같은 입력에도 응답이 달라질 수 있음을 뜻합니다.

### 자주 하는 오해

- **오해:** ROUGE나 BLEU 점수가 높으면 애플리케이션 배포를 승인해도 된다.
  **바로잡기:** 요약·번역 점수는 일부 품질만 봅니다. 사실성, 안전성, 사용자 만족도, 지연 시간, 상호 작용당 비용도 사용 사례에 맞게 평가합니다.

### 스스로 답하는 확인 질문

1. 요약과 번역에서 각각 어떤 자동 평가 지표를 우선 검토할 수 있나요?
2. 자동 평가와 인간 평가를 함께 사용해야 하는 이유는 무엇인가요?

### 공식 범위와 출처

- 공식 Task 3.4의 인간 평가, 벤치마크 데이터세트, Amazon Bedrock 모델 평가와 ROUGE·BLEU·BERTScore·LLM-as-a-judge 지표 범위를 다룹니다.
