...
- **한 줄 요약:** 생성형 AI 애플리케이션은 FM만으로 완성되지 않으며, 최신 근거를 위한 RAG와 인프라·도구·UI를 연결하고 KPI로 효과를 검토합니다.
- **선수 지식:** KPI는 업무 목표 달성 여부를 측정하는 지표이며, 예를 들어 작업 완료율·사용자 만족도·상호 작용당 비용을 사용할 수 있습니다.

### 자주 하는 오해

- **오해:** RAG는 재훈련보다 언제나 저렴하고 오래된 지식을 완벽하게 해결한다.
  **바로잡기:** RAG는 업데이트가 잦은 지식에 유용하지만, 데이터 수집·검색·권한·지연 시간 비용이 듭니다. 검색 결과와 생성 결과도 평가해야 합니다.

### 스스로 답하는 확인 질문

1. 최신 정책 Q&A에서 RAG를 먼저 검토할 수 있는 이유와 추가 운영 구성 요소는 무엇인가요?
2. 모델 품질이 좋아도 비즈니스 KPI가 나쁠 수 있는 사례를 하나 들어 보세요.

### 공식 범위와 출처

- 공식 Task 3.4의 RAG·에이전트·워크플로 애플리케이션 평가와 비즈니스 목표 정렬 지표 범위를 다룹니다.
