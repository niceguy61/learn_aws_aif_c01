...
- **한 줄 요약:** RAG는 검색기와 생성기를 결합해 최신·도메인 문서를 근거로 활용하며, 에이전트는 여러 시스템 호출이 필요한 작업 흐름을 조정합니다.
- **선수 지식:** 검색기는 질문과 관련된 문서를 찾고, 생성기는 찾은 문서를 포함한 입력으로 응답을 만듭니다.

### 자주 하는 오해

- **오해:** RAG를 적용하면 할루시네이션이 제거되고, 에이전트는 사람의 승인 없이 모든 작업을 맡겨도 된다.
  **바로잡기:** RAG는 근거 있는 응답을 돕지만 검색 오류와 생성 오류가 남을 수 있습니다. 에이전트의 도구 권한·입력·결과는 업무 위험에 맞게 통제하고 검증합니다.

### 스스로 답하는 확인 질문

1. 최신 사내 규정을 묻는 챗봇에 재훈련보다 RAG를 먼저 검토할 이유는 무엇인가요?
2. 단순 Q&A와 항공권 예약처럼 여러 API 호출이 필요한 업무는 에이전트 활용 측면에서 어떻게 다른가요?

### 공식 범위와 출처

- 공식 Task 3.1의 RAG 적용 사례, AWS 벡터 저장 옵션, AI 에이전트의 역할과 비즈니스 적용 범위를 다룹니다.
