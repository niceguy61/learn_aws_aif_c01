...
- **한 줄 요약:** 추론 파라미터는 응답의 다양성과 길이를 조절하고, RAG는 관련 외부 근거를 프롬프트에 더해 답변을 돕습니다.
- **선수 지식:** 임베딩은 텍스트·이미지의 의미를 숫자 묶음으로 표현한 것이고, 벡터 저장소는 이 숫자로 비슷한 정보를 빠르게 찾습니다.

### 자주 하는 오해

- **오해:** 온도를 낮추거나 RAG를 쓰면 답변이 항상 사실이 된다.
  **바로잡기:** 파라미터는 생성 방식만 조절하고, RAG 품질은 검색 문서·권한·청킹·검증에 좌우됩니다. 둘 다 사실성을 보장하지는 않습니다.

### 스스로 답하는 확인 질문

1. 창의적 광고 문구와 규정 안내 답변은 온도 설정을 어떻게 다르게 검토할 수 있나요?
2. RAG에서 벡터 저장소가 맡는 역할을 한 문장으로 설명할 수 있나요?

### 공식 범위와 출처

- 공식 Task 3.1의 추론 파라미터, RAG, 벡터 데이터베이스 내 임베딩 저장 범위를 다룹니다.
