...
- **한 줄 요약:** 가장 큰 FM이 항상 정답은 아니며, 업무의 모달리티·품질·비용·지연 시간·입출력 길이에 맞는 모델을 고릅니다.
- **선수 지식:** 추론은 학습된 모델이 새 입력에 답을 만드는 과정이고, 지연 시간은 요청부터 응답까지 걸리는 시간입니다.

### 자주 하는 오해

- **오해:** 정확도가 가장 높은 모델을 선택하면 된다.
  **바로잡기:** 실시간 응답, 예산, 언어·이미지 같은 입력 형태, 불균형 데이터에서의 적절한 지표도 함께 비교해야 합니다.

### 스스로 답하는 확인 질문

1. 실시간 고객 상담과 야간 보고서 요약은 지연 시간 요구가 어떻게 다른가요?
2. 불균형 데이터에서 정확도 하나만으로 모델을 판단하면 왜 위험한가요?

### 공식 범위와 출처

- 공식 Task 3.1의 모델 선택 기준(비용, 모달리티, 지연 시간, 다국어, 크기·복잡도, 사용자 지정, 입력·출력 길이)을 다룹니다.
